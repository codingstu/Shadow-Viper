...
def generate_fallback_battle_data(post_title: str, comments: List[Dict]) -> dict:
    ...
    for i, row in enumerate(comments):
        ...
        if is_red: team_red_warriors.append(warrior)
        else: team_blue_warriors.append(warrior)
...
async def analyze_comments_for_battle(post_title: str, comments: List[Dict]) -> dict:
    ...
    comment_rows = [row for row in comments if row.get('类型', '').upper() in [t.upper() for t in valid_types] or 'User:' in row.get('备注', '')]
    ...
    for index, row in enumerate(comment_rows[:30]):
        ...
        user_id = row.get('用户') or (row.get('备注', '').split('User:')[1].strip() if 'User:' in row.get('备注', '') else f"Warrior_{index}")
